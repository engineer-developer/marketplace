...
class AutoSchemaWithDeleteMethod(AutoSchema):
    """Переопределён метод для получения REQUEST BODY в методе DELETE"""

    def _get_request_body(self, direction="request"):
        """
        В кортеж добавлен 'DELETE' для возможности получения 'request body'
        """
